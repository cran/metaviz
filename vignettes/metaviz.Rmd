...
`"Purples"`. Color palettes are used from the package [**RColorBrewer**](https://CRAN.R-project.org/package=RColorBrewer). A grey rainforest plot is depcited below.
```{r, dev='CairoPNG', fig.align = 'center', fig.height = 6.5, fig.width = 6.5}
rainforest(x = mozart[, c("d", "se")], names = mozart[, "study_name"], 
...
The `rainforest` function can use the output of the function `rma.uni` from *R* package [**metafor**](https://CRAN.R-project.org/package=metafor), a widely used package for meta-analytic modelling. The package [**metafor**](https://CRAN.R-project.org/package=metafor) needs to be installed and then attached within the *R* environment.
```{r}
library(metafor)
```

We use the output of the `rma.uni` function as input for `rainforest`. First we compute a meta-analysis using `rma.uni` from package [**metafor**](https://CRAN.R-project.org/package=metafor).
```{r}
...
It is also possible to visualize a subgroup analysis using one categorical moderator which was conducted with [**metafor**](https://CRAN.R-project.org/package=metafor). First, we compute the subgroup analysis. 
```{r, dev='CairoPNG', fig.align = 'center', fig.height = 6.5, fig.width = 6.5}
...
The function `rainforest` uses the package [**ggplot2**](https://CRAN.R-project.org/package=ggplot2) to create the rainforest plots. Therefore, the output of `rainforest` is a [**ggplot2**](https://CRAN.R-project.org/package=ggplot2) object and can be modified using the [**ggplot2**](https://CRAN.R-project.org/package=ggplot2) syntax. We illustrate this with three examples below.

### Adding a cumulative meta-analysis
It is possible to plot additional data to the plot. We will add the results of a cumulative meta-analysis to the rainforest plot. First, we attach the [**ggplot2**](https://CRAN.R-project.org/package=ggplot2) package and then store a rainforest plot within a variable.
```{r}
library(ggplot2)
